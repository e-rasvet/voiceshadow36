...
//add_to_log($course->id, "voiceshadow", "deletelikes", "deletelikes.php?id=$cm->id", "$voiceshadow->id");


if (!empty($ids)) {
    $DB->delete_records("voiceshadow_likes", array("id" => $ids));
    
    $c = 0;
    
    if (!$lists = $DB->get_records_sql ("SELECT id FROM {voiceshadow_likes} WHERE fileid = ? ORDER BY time DESC", array($fileid))) {
...
$lists = $DB->get_records_sql ("SELECT id, userid FROM {voiceshadow_likes} WHERE fileid = ? ORDER BY time DESC", array($fileid));
...
foreach ($lists as $list) {
    ...
    $cell2     = new html_table_cell(get_string("like", "voiceshadow"));
    $cell3     = new html_table_cell(html_writer::link(new moodle_url('/mod/voiceshadow/deletelikes.php', array("id" => $id, "a" => "delete", "fileid" => $fileid, "ids" => $list->id)), get_string("delete", "voiceshadow"), array("onclick"=>"return confirm('".get_string("confim", "voiceshadow")."')")));
